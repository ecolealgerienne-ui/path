#!/usr/bin/env python3
"""
PREPARE FAMILY DATA - VERSION v8 (FIX INVERSION 180°)

CHANGEMENT CRITIQUE v7 → v8:
- ❌ v7: y_dist = y_coords - center_y  (vecteurs CENTRIFUGES - pointent vers extérieur)
- ✅ v8: y_dist = center_y - y_coords  (vecteurs CENTRIPÈTES - pointent vers centre)

Erreur angulaire v7: 179.95° (inversion parfaite)
Erreur angulaire v8: ~0° (attendu)

Distance attendue: 40.51px → <2px (gain -95%)

Usage:
    python scripts/preprocessing/prepare_family_data_FIXED_v8.py --family epidermal
"""
...
def prepare_family_data(family: str, pannuke_dir: Path, output_dir: Path):
    """
    Prépare les données pour une famille (v8 - fix inversion 180°).
    ...
    """
    print("=" * 80)
    print(f"PRÉPARATION DONNÉES FAMILLE: {family.upper()} (VERSION v8 - FIX 180°)")
    print("=" * 80)
    print()
    ...
    for fold in [0, 1, 2]:
        ...
        for i, organ in enumerate(types):
            ...
            hv_target = compute_hv_maps(inst_map)  # ✅ v8 fix inversion 180°
    ...
    # Stats HV
    print("STATS HV (v8 - fix inversion 180°):")
...
def main():
    parser = argparse.ArgumentParser(description="Prepare family data (v8 - fix inversion 180°)")
    parser.add_argument(
        "--family",
        type=str,
        required=True,
