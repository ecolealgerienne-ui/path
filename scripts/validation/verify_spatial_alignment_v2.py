#!/usr/bin/env python3
"""
VÉRIFICATION SPATIAL ALIGNMENT V2 (FIX Bug Fantôme - Fold Offset)

BUG IDENTIFIÉ (v1):
    - Comparait HV targets vs NP targets (tous deux depuis NPZ)
    - Ne chargeait JAMAIS le vrai masque GT PanNuke
    - Distance de 182px = comparaison entre patients DIFFÉRENTS

FIX (v2):
    - Charge fold_id + image_id depuis NPZ
    - Charge le VRAI masque GT depuis PanNuke/fold{fold_id}/masks.npy[image_id]
    - Compare centroides prédits (HV maps) vs vrais centroides (GT mask)

Usage:
    python scripts/validation/verify_spatial_alignment_v2.py --family epidermal
"""
...
def predict_centroids_from_hv(hv_map: np.ndarray, np_mask: np.ndarray) -> list:
    ...
    Méthode HoVer-Net:
    1. Calculer magnitude du gradient HV
    2. Détecter peaks locaux (centres des noyaux)
    ...
    Returns:
        list of (cy, cx): Centroides prédits
    """
    from skimage.feature import peak_local_max

    # Magnitude HV (énergie)
    energy = np.sqrt(hv_map[0]**2 + hv_map[1]**2)

    # Détecter peaks dans les régions de noyaux
    ...
    # Convertir en liste de (cy, cx)
    centroids = [(y, x) for y, x in peaks]
...
def main():
    parser = argparse.ArgumentParser(description="Vérification Spatial Alignment V2 (Fold-Aware)")
    parser.add_argument("--family", type=str, required=True,
                        choices=["glandular", "digestive", "urologic", "respiratory", "epidermal"])
    parser.add_argument("--n_samples", type=int, default=5)
    parser.add_argument("--output_dir", type=str, default="results/spatial_alignment_v2")
    args = parser.parse_args()

    print("="*80)
    print("VÉRIFICATION SPATIAL ALIGNMENT V2 (FOLD-AWARE)")
    print("="*80)
